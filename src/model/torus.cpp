...
namespace model {
    ...
    // Internal helper struct for interleaving vertex data
    struct Vertex {
        float position[3];
        float normal[3];
        float uv[2];
    ...
    TorusMesh makeTorusGrid(int columns, int rows, float outerRadius, float innerRadius) {
        ...
        std::vector<Vertex> vertices(static_cast<size_t>(vertexCols) * vertexRows);

        // Helper lambda to map 2D grid coordinates to linear index
        auto vertexIndex = [&](int i, int j) -> int {
            return j * vertexCols + i;
            };
        ...
        for (int j = 0; j < vertexRows; ++j) {
            ...
            for (int i = 0; i < vertexCols; ++i) {
                ...
                vertices[static_cast<size_t>(vertexIndex(i, j))] = vertex;
            }
        }
